...
| `/chat`'s sidebar adds a Channels section beside Direct Messages; the thread pane drops the "Active" presence dot and the call/video icons | Nexus's `/apps/chat` reference is DM-only, no channels — [20-team-chat.md §6](20-team-chat.md) adds channels because a channel and a DM are both just "a conversation" per that spec's §2. Presence is ephemeral state, not a durable Postgres fact worth a row, and call/video implies a telephony/WebRTC integration outside this spec's scope — both flagged as excluded there (§8), not silently dropped |
