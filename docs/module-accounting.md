...
_Status: **DONE** (2026-09-21), manual reconciliation only. Bring-your-own-key, not Stripe Connect: `/accounting/payment-gateway` lets a tenant paste their OWN Stripe secret key — used only against that tenant's own account, never Kaaj's own `PRIVATE_STRIPE_API_KEY` (the unrelated integration wired to `(admin)/account/billing`). The key is validated against a real Stripe call (`balance.retrieve()`) before it is ever sealed, then stored via `$lib/server/pii`'s `subjectType: "tenant"` — the same mechanism `bank_accounts`' `_ct` columns are meant to use, and this is its first live app-action consumer. A new `payment_gateway_settings` table, not columns on `tenants`: the tenant row is broadly readable, and a Stripe secret key needs the same finance-only row-visibility `bank_accounts`/`recurring_schedules` already have (`app.reads_all_accounting()`/`writes_accounting()`), which only a dedicated table's own RLS policy can give it. At issuance, and via a "Create payment link" button that only ever fills a gap (refused once `payment_url` is already set — no app path replaces an existing link), `createInvoicePaymentLink()` creates a Stripe **Payment Link** — deliberately not a Checkout Session, whose URL expires within 24 hours (Stripe's own cap), long before a `net_30` invoice is even due; a Payment Link has no expiry, so it is the only kind safe to store on the invoice and embed in a PDF/email that may be opened weeks later. Built from a one-off Price (Stripe Payment Links need a real Price id, not inline pricing) sized via integer whole/fractional-part arithmetic, never `Number(amount) * 100` — the same reasoning CLAUDE.md's Money section gives for JS float rounding — and refuses outright for Stripe's three-decimal currencies (BHD/JOD/KWD/OMR/TND) rather than silently mis-scaling them 10x; Stripe's zero-decimal currencies (JPY etc.) pass through unmultiplied. Best-effort at issuance — a Stripe failure surfaces as a warning alongside "Issued", never blocks posting real revenue to the ledger — and the eligibility/network-call/DB-write split follows US-ACC-008's own lesson: reading the sealed key and the invoice is one short transaction, the Stripe call happens with no transaction open, and the write + audit entry are a second short transaction. **Manual reconciliation only** (explicit product decision, not a gap): no webhook, so a completed Stripe payment does not auto-post — a human still runs "Record payment" as before; the link's amount is frozen at whatever was owed at creation time, so a credit or partial payment posted afterward leaves it charging the original figure — with no remedy through the product: the one-link-per-invoice guard means it can't be regenerated, and deactivating the stale link means going into Stripe directly. Auto-reconciliation would need a "system" actor this codebase has no precedent for (every existing automated-looking write — recurring invoice generation, accrual posting — is actually a human clicking "apply now") and a service-role-quarantined webhook endpoint; deferred as its own, separately-scoped effort. Caught and fixed before shipping, not after: the fixture's blanket "no empty column" sweep had filled every invoice with a fake `payment_url`/`'GoCardless'`/`'Stripe'` gateway label nothing had ever configured — the same L87/L89 shape a third time, recorded as [L90](10-lessons-learned.md) and fixed by removing the sweep for these columns and seeding one deliberately-chosen example shaped like this feature's real output instead. Tested in `stripe_gateway.test.ts` (key format, currency conversion, a real `Stripe.errors.StripeAuthenticationError` instance distinguished from a generic failure, Price+Payment-Link creation, all via a mocked Stripe client — matching this codebase's own posture for Resend), `payment-gateway/page.server.test.ts` (real seal/open round trip, both RLS negatives — a different tenant and a non-finance employee), and `invoices/[id]/page.server.test.ts` (skip/success/failure outcomes for link creation). Live-verified in a real browser: a malformed key refused before any network call, a well-formed key genuinely rejected by Stripe's real API with a distinct message, the seeded demo link rendering as a clickable "Payment link" on the invoice page and as a real "Pay this invoice online" link in the rendered PDF, and the honest "No Stripe key is configured" refusal on an eligible invoice with nothing configured._
...
_Status: **DONE** (2026-09-21), CSV only, no PDF — a deliberate, permanent scope boundary, not an oversight: browser print-to-PDF remains the path for a document-shaped copy, the same as every other report screen, and the story's "Excel or PDF" is answered with a format every spreadsheet tool already opens natively. All eleven report pages now have a sibling `/export` GET route behind the same `accounting.read` gate as the screen, downloading `text/csv` with `Content-Disposition: attachment`: Trial Balance, Profit & Loss, Balance Sheet, Cash Flow, AR Aging, AP Due Soon, Tax Summary, Customer Balances, General Ledger, Changes in Equity, FX Revaluation.

Filter parsing (`$lib/server/accounting/report_filters.ts`) is factored into one function per report filter shape — `parseAsOfCompareFilters`, `parsePeriodFilters`, `parseAsOfOnlyFilters`, `parseFromToFilters`, `parseLedgerFilters`, `parseApDueSoonFilters` — shared by each report's `load()` and its export route, so a download's `as_of`/`from`/`to` can never silently drift from what's on screen. The general ledger's `status` filter vocabulary (`LEDGER_STATUSES`) moved out of the page into `accounting.repo.ts` for the same reason a plain-`text` vocabulary belongs in one place (CLAUDE.md) — the page and its export both import it now, rather than each declaring its own copy that could drift.

Money cells hold the raw `NUMERIC` string, never `money()`'s locale-formatted output; the currency is named in the column header instead (base currency for the base-currency reports, per-row for AR aging/AP due soon/customer balances, which never convert). `$lib/server/accounting/csv.ts`'s `toCsv()` escapes CSV formula injection (a cell starting with `=`/`+`/`-`/`@`/tab/CR gets a leading `'`) without touching a legitimate negative or positive money value — tested directly, including the "still escapes AND quotes" case where a cell needs both, and the two-sided pair (`-2+3` escaped, `-1234.56` not) that proves the `Number.isNaN` guard discriminates rather than just existing.

**Comparison export** (closed 2026-09-21, previously a gap): Trial Balance and Balance Sheet always emit their comparison columns (`Compare Debits`/`Compare Credits`, `Compare Amount`/`Change`), left blank rather than swapped out of the header when no `compare_as_of` is given — a stable column set per report, not one that varies by query and silently breaks whatever a saved spreadsheet template or script expected. Profit & Loss, Cash Flow and Changes in Equity have no per-account comparison breakdown server-side (`profitAndLossComparison()`/`cashFlowComparison()`/`equityComparison()` return period TOTALS only, the same shape their own screens show) — their exports always append the same named comparison summary rows, blank when `compare` is "none". Equity shipped with this built in from the start; the other three had it retrofitted.

**Row caps** (closed 2026-09-21, previously a gap for two of the five new reports): AP Due Soon and FX Revaluation refuse outright past `REPORT_ROW_CAP` (5,000 rows) — `bills` and `invoices` are both `SCALE_SENSITIVE`, and neither route has a cheap combined pre-count (FX revaluation runs two independent AR/AP queries with no shared `WHERE` to reuse), so both refuse AFTER fetching. The General Ledger export refuses BEFORE fetching instead, on the real `countLedgerLines()` result — a genuine `DOCUMENT_CHILD_CAP`-shaped pre-count, unlike the other two — because bounding by ENTRY count alone would be wrong: each entry can carry up to `DOCUMENT_CHILD_CAP` (500) lines of its own, so a 5,000-entry cap could still produce a 2.5M-row file. AR Aging and Customer Balances carry no cap at all — both group down to (customer, currency), bounded by the tenant's own `NOT_SCALE_SENSITIVE` customer roster, not by transaction volume — and Tax Summary and Changes in Equity are bounded by a small, fixed account/jurisdiction set.

Tested in `csv.test.ts` (the escape/quote rules), `report-export.test.ts` (all eleven routes: the refused-actor and portal-contact cases, real comparison figures flowing through when `compare`/`compare_as_of` is active and blank when it isn't, the GL's line-flattening), and a dedicated `row-cap.test.ts` per capped route (AP Due Soon, General Ledger, FX Revaluation) that mocks `REPORT_ROW_CAP` down to actually drive the refusal (L48) rather than trusting it untested. Live-verified in a real browser against real fixture data: Trial Balance's `$217,381.53` total, AR Aging's two real currencies (USD/GBP) in separate rows, AP Due Soon's single eligible bill, and Profit & Loss's genuinely negative net income (`-56920.00`) flowing through the `-` escape branch unescaped._
...
- [ ] Recurring invoices/bills. US-ACC-004 (PARTIAL as of 2026-09-20 —
...
- [x] Online payment links (Stripe) on customer invoices — DONE
      (2026-09-21), manual reconciliation only. US-ACC-002. Bring-your-own-
      key per tenant via `/accounting/payment-gateway`, distinct from Kaaj's
      own Stripe integration (`(admin)/account/billing`, one shared
      platform account). A Stripe Payment Link — not a Checkout Session,
      which expires within 24 hours — created at issuance and via a retry
      button, best-effort against the ledger posting. No webhook: a human
      still records the payment; auto-reconciliation is deferred pending a
...
- [x] Report export (CSV) — DONE (2026-09-21), CSV only, no PDF (deliberate
      scope boundary). US-ACC-045. All eleven report pages have a sibling
      `/export` route; comparison columns/rows are exported for real where a
      report has one, and the three routes whose driving table is
      `SCALE_SENSITIVE` (AP Due Soon, General Ledger, FX Revaluation) carry a
