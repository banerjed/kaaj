...
6. ~~**Tax rates as their own accounting concept (§4)**~~ — done 2026-09-13
   (US-ACC-046): a real `tax_rates` table now exists and every accounting tax
   FK resolves to it, not `payroll_tax_rates`. A configuration route and an
   actual rate lookup (invoices/bills still take a typed amount) remain.
