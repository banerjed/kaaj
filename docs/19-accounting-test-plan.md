...
- Manual journal entries require the same balancing/period/permission checks as system-generated ones — no backdoor. **[DONE]** (2026-09-13, US-ACC-034)
  *`/accounting/journal-entries/new` (`acc.recordManualJournalEntry()`) doesn't reimplement the checks — it resolves the picker's account ids to codes in one query, then delegates straight to `postJournal()`, the same posting engine `issueInvoice`/`approveBill`/`recordPayment` already share, so the balancing/period rules are exercised by a THIRD caller rather than trusted by construction. `accounting.writes.test.ts` ("recording a manual journal entry") covers what's new here specifically: `source_type`/`source_id` are `"manual"`/`null`; the account-id-to-code resolution posts to the right account (break/revert-verified — flipping the guard's refusal reason made the "bad account id" test fail as expected, then reverted); a stale/nonexistent account id is refused distinctly from a bad code (one step earlier than `postJournal`'s own `accountId()` lookup, since the picker sends an id); and posting into the fixture's closed January 2026 period is refused the same as any other caller. The balancing and multi-line guards themselves are `postJournal`'s own, already covered directly in "posting a journal entry directly" — not re-asserted here, except for one case genuinely new to this caller: three lines of `0.05`/`0.02`/`0.03` at exchange rate `1.1` balance in native currency but not after `postJournal`'s per-line rounding to base (`0.06` vs `0.05`) — unreachable through `issueInvoice`/`approveBill`, whose lines come from computed totals, but real the moment amounts and a rate are both free-form. `postJournal`'s `does_not_balance` refusal now reports the native and base sides as two distinct messages rather than one, asserted directly on `e.detail`. `form-errors.spec.ts` adds three cases specific to the form: fewer than two lines, a line with both a debit and a credit, and a mismatched total (asserting the actual figures appear in the message, not just that it failed).*
...
- Reopening a locked period is either disallowed or requires elevated permission + is itself audited. **[DONE]** (2026-09-13, US-ACC-035, `INV-ACC-002`)
  *`/accounting/periods` (`acc.closePeriod()`/`acc.reopenPeriod()`) now writes `accounting_periods.status` for real — a period's own row, not just `postJournal`'s existing read-only check of it. Close: `open` → `closed` only. Reopen: `closed` → `open` only, with a required `reason` field — a `locked` period is explicitly DISALLOWED from reopening, since nothing in this codebase writes that stronger status either (the fixture's one locked row, December 2025, models a hypothetical future lock step this repo hasn't built); reopening one would mean guessing at a ceremony with no real lock workflow to observe. This is still true of `packages/database/fixtures/mock-data.sql:877`'s own `audit_log` row, and it is now MORE specifically wrong than before: that row records action `close_period` with `{"status": {"from": "closed", "to": "locked"}}` — a lock, not a close — but the real `close_period` action shipped here only ever performs `open` → `closed`. The action name is now shared between a real write and a fixture row describing a transition that action never performs — but the fixture row itself is not wrong: it is internally consistent with the rest of the fixture, which is the record of HOW December 2025 reached `locked` (a genuinely `locked` row), not a claim about what the real `close_period` action does today. Left as-is deliberately; what is missing is code that performs a LOCK, not a correct fixture. Noted here so nobody reads that row as evidence the shipped close action can reach `locked`. Both actions are `accounting.write`-gated and audited (`close_period`/`reopen_period`) — no new `@kaaj/authz` permission, matching how `voidInvoice`/`recordWriteOff` already combine an existing permission with a mandatory reason and an audit entry rather than a narrower write permission. `spec-tests`' `INV-ACC-002` still asserts the *desired* rule as its own pure-function model (`canMutateAccountingPeriod`, deliberately a separate vocabulary from `@kaaj/authz` per CLAUDE.md's "keep the two suites independent") — this closes the gap in the OTHER direction: real code now exists for `authz-conformance.spec.test.ts`-style comparison to eventually check against, though no such comparison has been added (out of scope for this increment; that bridge today only covers sensitive-data-read decisions, not this write). Tested in `accounting.writes.test.ts` ("closing and reopening an accounting period", 9 cases): a real close writes `closed_by`/`closed_at`; closing a non-open period is refused; a real reopen clears both columns; reopening an open period is refused; reopening the fixture's locked period is refused; a nonexistent period id is refused for both; an auditor (reads everything, writes nothing) passes the read but the write comes back refused rather than a silent no-op (L47/L68) — break/revert-verified by temporarily discarding the `RETURNING id` check and confirming exactly that test fails; and two composition cases that close/reopen alone don't prove — closing a period actually stops a subsequent posting into it, and reopening one actually lifts that refusal again — each break/revert-verified by disabling `postJournal`'s own period check and confirming the composition test fails alongside the pre-existing period-closed tests in `receivables.writes.test.ts`, while the reopen-composition case keeps passing (consistent with "the check no longer runs at all", not a false positive). `accounting.test.ts` adds "accounting periods" (2 cases): `closed_at` returns as a real `Date` for the fixture's closed rows, not a `::text` cast that `instant()` isn't guaranteed to parse identically on every engine; and finance-only RLS. `form-errors.spec.ts` covers closing a non-open period, reopening with no reason, and reopening a non-closed period.*
...
- Year-end close correctly zeroes out revenue/expense accounts into retained earnings/equity. **[DONE]** (2026-09-13, US-ACC-051)
  *`/accounting/year-end-close` (`acc.previewYearEndClose()`/`acc.yearEndClose()`) computes each revenue/expense account's cumulative balance as of a chosen date and posts a single balanced entry zeroing every nonzero one, with net income/loss going to Retained Earnings (`3000`). The preview and the post are two separate requests, so the confirm form carries the previewed net income as a hidden field and `yearEndClose` refuses (`allocation_mismatch`) if what it recomputes at post time no longer matches — the same shape as a lockbox batch's own `allocation_mismatch`, checked in SQL/NUMERIC. The check is on the bottom-line net income only, the same level of rigor as the lockbox check on its total: it guarantees the confirmed net-income figure is exactly what posts to Retained Earnings, not that every individual revenue/expense line is unchanged — an offsetting pair posted in between (a revenue and an expense of the same size) leaves net income identical while changing which accounts get zeroed. Every figure was verified independently against the real database via `psql` BEFORE writing any TypeScript or tests — net income as of `2026-12-31` (`-56920.00`) matches `balanceSheetComparisonTotals`'s own already-trusted figure for the same date exactly, and the per-account breakdown sums to the same total. Tested in `accounting.writes.test.ts` ("year-end close", 7 cases): the preview matches those verified figures exactly; a real close posts the correct debit/credit on every line, including Retained Earnings taking a DEBIT for a net loss (equity decreases); a preview taken right after the close confirms the accounts really are back at zero rather than just trusting the post's own claim; running it twice at the same date finds nothing left the second time (`no_lines` — the first close's own lines are posted activity too, so this is idempotent by construction, not a flag); a stale/mismatched confirm is refused; posting into the fixture's closed January period is refused; and finance-only RLS. The debit/credit sign rule (revenue's positive net is zeroed by a DEBIT, expense's positive net by a CREDIT — the two trade sides, which is easy to get backwards) is break/revert-verified: flipping which account type triggers which side made the two amount-checking tests fail as expected, one of them with `does_not_balance` since an inverted rule doesn't just mislabel a line, it makes the entry stop balancing. The mismatch guard is separately break/revert-verified: forcing its SQL comparison to always report no mismatch made the stale-preview test fail with "expected a refusal ... and the write succeeded". `form-errors.spec.ts` covers nothing-to-close, a date in a closed period, a stale confirm, and — separately — that a refusal actually re-fetches fresh data: SvelteKit's `use:enhance` only re-runs `load` on a SUCCESSFUL submit by default, so without a fix a mismatch refusal would leave the confirm form holding the very figures just refused, and resubmitting would repeat the refusal forever; fixed by calling `invalidateAll()` in the form's own submit handler regardless of outcome, and verified on the wire (a `__data.json` request follows the refusal only when the fix is present — a DOM assertion on the stale field can't tell the two cases apart, since nothing in this read-only suite changes the real figure, so Svelte never rewrites the field either way). A forward-dated `asOf` is deliberately permitted, not refused — the balance sheet, trial balance and P&L pages already report "as of" any date, past or future, purely from what's posted by then, and nothing in this codebase ties a report's `asOf` to wall-clock "today". Not gated on any period being closed first, or on a close-checklist — that gate still doesn't exist (§13) — so this is exactly as safe/unsafe as any other `postJournal` caller, no more.*
...
- Comparative trial balance, two independent `asOf` dates side by side. **[DONE]** (added 2026-09-13, US-ACC-041 remainder)
  *`trialBalanceComparison()`/`trialBalanceComparisonTotals()` — a genuinely different shape from the P&L/cash-flow/equity period comparisons, which compare a current window to a computed prior one; a trial balance is cumulative, so it names a second point in time directly instead. `compare_as_of` may fall before OR after `as_of` — unlike the cash flow statement's `from`/`to`, these are two independent snapshots, not a range, so there is no inverted-order case to refuse. Tested in `accounting.test.ts` ("trial balance comparison"): each date evaluated independently (a date before any posting reads a real zero, not the other date's total), the same-date-both-sides identity against `trialBalance()`'s own single-date figures, and finance-only RLS. `/accounting/trial-balance` gained a "Compare to" field and an additive comparison table; `form-errors.spec.ts` ("a trial balance comparison date with no 'as of' date is refused") covers the `compare_as_of`-without-`as_of` 400, and `smoke.spec.ts` loads the page with both dates set so the comparison table itself renders under test.*
...
- Credit memos and refunds reduce AR and revenue symmetrically to how the original invoice increased them. **[DONE]** (added 2026-09-13)
  *`acc.recordCreditMemo()` posts Dr Revenue / Cr AR — the exact mirror of `issueInvoice()`'s Dr AR / Cr Revenue — via a new `invoice_credits` table and `invoices.amount_credited`/`base_amount_credited` columns, with `ck_invoices_amounts_reconcile` widened to `amount_due = total - amount_paid - amount_credited`. Refuses a credit larger than the invoice's own current balance (`over_credit`) and sets a new `credited` status only when the credit brings the balance to exactly zero, leaving a partial credit's existing status untouched. New UI on `/accounting/invoices/[id]` ("Issue a credit"). Tested in `receivables.writes.test.ts` ("issuing a credit memo"): the ledger stays balanced, the balance drops by exactly the credited amount, both status-transition branches are exercised, and both guards (over-credit, draft) are break/revert-verified.*
...
- Per-customer aggregate balance (how much each customer currently owes, for credit-risk review) is available, not just per-invoice figures. **[DONE]** (added 2026-09-13 — this bullet is new, not a status flip from the original v2.1 audit, which covered the underlying gap under US-ACC-019 in `module-accounting.md` rather than as its own line here)
  *`acc.customerBalances()` at `/accounting/customer-balances` groups open invoices by `(customer_id, currency)`, with invoice count, invoiced/paid/credited/due totals (`total_credited` added 2026-09-13 alongside credit memos, so the four figures reconcile visibly), and the customer's `credit_limit`. Takes no `asOf` — a live balance, not a point-in-time report — so it always reads the invoices' current `amount_due`. Tested in `accounting.test.ts` ("customer balances"): `total_due = total_invoiced - total_paid - total_credited` is asserted against real fixture figures (break/revert-verified), a customer with only a draft invoice is excluded, and finance-only RLS is checked. The page's over-limit flag is a `compareDecimal` comparison rather than a new SQL column — the fixture's uniform `100.00` credit limit on every customer means only the over-limit branch is exercised by real data, which is noted rather than hidden.*
- Write-off of bad debt moves the balance out of AR into an expense/allowance account and is auditable. **[DONE]** (added 2026-09-13)
  *`acc.recordWriteOff()` posts Dr Bad Debt Expense (`5500`, added to the fixture chart of accounts) / Cr Accounts Receivable, sharing `invoice_credits` and the reversing-entry mechanism with credit memos via a `credit_type` column (both call a shared `recordInvoiceCredit()`). Sets `written_off` only when the balance reaches exactly zero, refuses more than the invoice's own balance (`over_writeoff`), and records a `record_writeoff` audit entry. Tested in `receivables.writes.test.ts` ("writing off bad debt", 7 tests): the journal entry's account/amounts, the reduction in `amount_due`, the `written_off` vs. untouched-status branches, and a mixed credit-memo-plus-write-off case proving `invoice_credits`/`amount_credited` sum both kinds correctly. The shared guard body was already break/revert-verified by the credit-memo increment; what's new here is the `recordWriteOff()` wrapper's own routing, verified by swapping its `overAmountReason` and `settledStatus` to the credit-memo values and watching the write-off-specific assertions fail on the wrong reason and the wrong status, before reverting both.*
...
### 3.3 Revaluation (period-end) — **[PARTIAL]** (2026-09-18), report-only
...
- Tax calculated matches the jurisdiction's rate table for the transaction date; rate changes apply prospectively. **[PARTIAL]**
  *Line-level tax math is exercised via fixture data — `payables.test.ts:45` ("has a bill carrying tax, so the tax path is exercised") and `receivables.writes.test.ts:127` (exact split, `3214.97` tax / `36225.00` revenue) — but these check internal consistency (`total = subtotal + tax`) against whatever `tax_amount` the fixture already carries, not that a rate was looked up for the transaction date from a rate table — invoices/bills still take a manually-typed tax amount. As of 2026-09-13 (US-ACC-046) there IS a dedicated accounting `tax_rates` table, and all five accounting tax FKs (`invoice_lines.tax_rate_id`, `bill_lines.tax_rate_id`, `chart_of_accounts.tax_rate_id`, `journal_entry_lines.tax_rate_id`, `customers.tax_rate_id`) resolve to it — `20260913223213_accounting_tax_rates.sql`, confirmed via `pg_constraint.confrelid` and guarded by a new `verify-stories.sql` check (`US-ACC-046-fk`) that reads those same five constraints. Previously every one of them pointed at `payroll_tax_rates(id)`, a payroll income-tax table. `/accounting/tax-rates` (added the same day) now lets finance create a rate and deactivate/reactivate one, tested against the real database in `tax_rates.writes.test.ts` (6 cases, including finance-only RLS and a break/revert-verified guard against a nonexistent id silently succeeding). As of 2026-09-13 (US-ACC-048/049), invoice/bill create forms carry an optional per-line "Tax rate" picker that attaches `tax_rate_id` to the line — a reference only, so `issueInvoice`/`approveBill` can attribute the tax to a jurisdiction when posting. What's still missing: editing a rate's other fields once created, and any code path that computes the tax amount from the rate rather than trusting what's typed in.*
- Tax-exempt customers/products correctly produce zero tax, and an exemption expiring mid-period stops exempting after expiry. **[PARTIAL]** (2026-09-13)
  *`customers.tax_exempt_until` (nullable `DATE`; `NULL` = indefinite) added in `20260913230000_customer_tax_exempt_until.sql`. Enforced twice, deliberately: `createInvoice` refuses a taxed line for a customer exempt as of the invoice's own date (not "today"), and `issueInvoice` — the step that actually posts `tax_total` — repeats the check independently against the invoice's stored date and a fresh read of the customer row, since a draft can be created before an exemption is set or issued after one has expired, neither of which `createInvoice`'s check alone would catch. `accounting.writes.test.ts` ("tax-exempt customers", 4 cases) covers a taxed line refused within the window, a zero-tax line allowed within the window, a taxed line allowed once the exemption has expired, and — the case motivating the second check — a draft created for a non-exempt customer that becomes exempt before issuing, refused at `issueInvoice`; that last guard was break/revert-verified (disabling it let the write silently succeed). `form-errors.spec.ts` covers the same refusal end-to-end. Products/services are moot — there is no per-product tax flag anywhere in this schema. Still missing: no UI to set either column on a customer (fixture- or direct-database-only, same posture as `chart_of_accounts.tax_rate_id`), and the invoice form's customer picker doesn't surface exemption status before a line is typed.*
...
- A tax liability report ties to the sum of all tax-account postings for the period. **[PARTIAL]** (2026-09-13)
  *`/accounting/tax-summary` (`acc.taxLiabilitySummary()`) sums output tax (`"2200"` credits) and input tax (`"1200"` debits) grouped by `tax_rate_id`, for an optional `from`/`to` period — read directly from `journal_entry_lines`, so it ties to postings by construction rather than by a separate reconciliation check. Getting a real jurisdiction to show up at all required `issueInvoice`/`approveBill` to start grouping taxed lines by rate and posting `tax_rate_id` onto the GL line (previously the column existed but nothing wrote it), fed by a new optional per-line tax-rate picker on the create forms. Tax posted with no rate attached is its own explicit, labelled row rather than silently merged into a real jurisdiction. Tested in `accounting.writes.test.ts` ("tax liability by jurisdiction", 5 cases) — grouping by rate rather than lumping into one line (break/revert-verified), the summary's own arithmetic against real posted activity, the unattributed bucket, and — deliberately not synthetic — an assertion against the committed fixture's one real posted tax line (`BILL-AWS-2026-01`, tagged with its rate via a targeted `UPDATE`, not a blanket backfill). `INV-ACC-006`'s `summarizeTaxLines` (output tax − input tax) remains a separate pure-function unit test in `packages/spec-tests` against hand-built objects; the two aren't meant to converge, since the app version does the arithmetic in SQL over real rows. Still missing: no VAT-return-shaped export, and reverse charge will post no GL line at all under `postJournal`'s existing zero-nets-out-of-the-entry rule — noted below, not yet designed around.*
...
6. ~~**Tax rates as their own accounting concept (§4)**~~ — done 2026-09-13
   (US-ACC-046/048/049): a real `tax_rates` table exists, every accounting tax
   FK resolves to it, a configuration route (`/accounting/tax-rates`) exists,
   and a jurisdiction-grouped tax liability report (`/accounting/tax-summary`)
   reads real posted GL activity. A rate-driven tax *amount* computation
   (invoices/bills still take a typed amount, now optionally tagged with a
   rate) remains, along with reverse charge (US-ACC-047) and a VAT-return
   export shape.
