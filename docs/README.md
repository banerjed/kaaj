...
15. **[10-lessons-learned.md](./10-lessons-learned.md)** - **Lessons learned** (running document)
    - The traps that cost real debugging time, each of which failed silently
    - Tenancy and auth (L1-L8), daisyUI/Tailwind/Nexus (L9-L18), process (L19-L21)
    - Read before building a new module; append when something bites

16. **[09-build-plan.md](./09-build-plan.md)** - Build plan: Firm Profile, then Employee Profile
    - The four pre-flight findings that shape the work (stale ADR-003 prose, `app_user`
      has no password, no seeded login carries a tenant claim, PostgREST is unreachable)
    - Phase checklist, ticked as steps are verified — resume here
    - What is deferred deliberately, and why

### API Surface

17. **[11-module-roadmap.md](./11-module-roadmap.md)** - What is left, and in what order
    - Dependency order, not specification order — every module reads what the one above writes
    - Phase 3 compensation through Phase 8 support surfaces
    - Why PII encryption must be resolved *before* payroll

18. **[12-beta-deployment.md](./12-beta-deployment.md)** - Running the beta in the cloud
    - Why not serverless: this app holds a Postgres pool and needs transaction-scoped session state
    - The two manual steps migrations cannot do, both of which fail silently
    - What a beta must not carry yet (PII encryption, tenant-only authorization)

19. **[api-surface.md](./api-surface.md)** - **Authoritative interface contract** (generated)
...
20. **[data-models/schema.sql](../packages/database/reference/schema.sql)** - **Authoritative database schema**
...
21. **[data-models/mock-data.sql](../packages/database/fixtures/mock-data.sql)** - Test organization
...
22. **[data-models/SCHEMA-RECONCILIATION.md](./data-models/SCHEMA-RECONCILIATION.md)** - Schema Reconciliation
