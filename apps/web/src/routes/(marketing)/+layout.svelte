...
<!--
  The CMSaasStarter theme, scoped rather than global.

  daisyUI resolves a theme from the nearest `data-theme` ancestor. Since the
  (app) product surface uses Nexus's themes on <html>, the marketing site has to
  claim its own or it would inherit whichever theme the app last selected. See
  the note at the top of src/app.css.

  `display: contents` on the wrapper means it generates no box at all, so the
  page's flex layout is unchanged; custom properties still inherit through it.
-->
  ...
  <div class="">
    ...
    >
      ...
      <aside>
        ...
        <a class="max-w-[260px]" href="https://getkiln.ai" rel="noreferrer">
